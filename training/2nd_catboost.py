...
print(
    f"train: {train_dates[0]} ~ {train_dates[-1]} ({len(train_dates)}일) {len(train_df):,}행"
)
print(
    f"valid: {valid_dates[0]} ~ {valid_dates[-1]} ({len(valid_dates)}일) {len(valid_df):,}행"
)
print(
    f"test : {test_dates[0]} ~ {test_dates[-1]} ({len(test_dates)}일) {len(test_df):,}행"
)
...
def add_pattern_features(train_base, target_df):
    ...
    result = result.merge(
        route_stop_peak_stat, on=["route_name", "staOrd", "is_peak"], how="left"
    )
    ...
    result = result.merge(
        route_time_stat, on=["route_name", "dayofweek", "hour"], how="left"
    )
...
print(
    f"피크 - train: {X_train_peak.shape} / valid: {X_valid_peak.shape} / test: {X_test_peak.shape}"
)
...
test_pred_full, test_proba_full = evaluate_classification(
    cat_full_cls, X_test, y_test_full, "TEST 만차 이진", class_labels=FULL_BINARY_LABELS
)
...
# 모델 저장
joblib.dump(cat_reg, os.path.join(ARTIFACT_DIR, "catboost_reg.pkl"))
joblib.dump(
    cat_peak_cls, os.path.join(ARTIFACT_DIR, "catboost_peak_congestion_cls.pkl")
)
joblib.dump(cat_full_cls, os.path.join(ARTIFACT_DIR, "catboost_full_cls.pkl"))
...
# 혼잡도 기준 저장
with open(
    os.path.join(ARTIFACT_DIR, "label_definition.json"), "w", encoding="utf-8"
) as f:
    json.dump(
        {
